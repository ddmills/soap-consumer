...
if( $_SERVER['argc'] < 2 ) {
  die("usage: wsdl2php <wsdl-file> [<directory>]\n");
}

$wsdl = $_SERVER['argv'][1];
$directory = isset($_SERVER['argv'][2]) ? $_SERVER['argv'][2] : '';
...
$code ="";
    ...
    if($pos !== false)
    {
        $code = "namespace ".substr($directory, 0, $pos).";\n\n";
    }
    elseif ($directory != '')
    {
        $code = "namespace ".$directory.";\n\n";
    }*/
...
foreach($service['types'] as $type) {
  ...
  $acumatica_commands =
          array("EveryValue", "RowNumber", "Field", "Value", "Action", "Key", "Answer", "NewRow", "Parameter");

  $code .= "class ";
  if(in_array($type['class'], $acumatica_commands))
  {
      $code .= $type['class']." extends Command {\n";
  }
  else
  {
      $code .= $type['class']." {\n";
  ...
  if( $type['class'] == 'Command')
  {
      $code .= "  //Workaround for PHP BUG 50675\n";
      $code .= "  function __clone(){\n";
      $code .= "    foreach(\$this as \$name => \$value){\n";
      $code .= "        if(gettype(\$value)=='object'){\n";
      $code .= "            \$this->\$name= clone(\$this->\$name);\n";
      $code .= "        }\n";
      $code .= "    }\n";
      $code .= "  }\n";
